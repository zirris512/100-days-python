...
def count_down(count: int):
    ...
    if count > 0:
        ...
        if reps % 2 == 0:
            sessions = int(reps / 2)
            checkmark.config(text=f"{'✔' * sessions}")
...
header = Label(
    text="Timer", font=(FONT_NAME, 40, "bold"), justify="center", fg=GREEN, bg=YELLOW
)
...
checkmark = Label(font=(FONT_NAME, 14), bg=YELLOW, fg=GREEN)
